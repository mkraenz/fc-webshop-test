...
export async function fetchSubcategories() {
  try {
    const data = await sql<DbProductSubcategory[]>`
      SELECT productsubcategoryid as id, * FROM production.productsubcategory 
      ORDER BY name asc
    `;

    const subcategory = data.map((subcategory) => ({
      id: subcategory.id,
